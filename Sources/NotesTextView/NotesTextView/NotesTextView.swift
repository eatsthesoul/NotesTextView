...
public class NotesTextView: UITextView {
    ...
    private lazy var proxyDelegate = MulticastTextViewDelegateProxy()
    
    public weak var clientDelegate: UITextViewDelegate? {
        didSet {
            // При изменении clientDelegate, обновляем externalDelegate в нашем прокси.
            proxyDelegate.externalDelegate = clientDelegate
        }
    }
    ...
    struct TextState: @unchecked Sendable {
    ...
    private func commonInit() {
        ...
        proxyDelegate.internalDelegate = self
    }
...
class MulticastTextViewDelegateProxy: NSObject, UITextViewDelegate {
    
    weak var internalDelegate: UITextViewDelegate?
    weak var externalDelegate: UITextViewDelegate?
    
    func textViewDidChange(_ textView: UITextView) {
        internalDelegate?.textViewDidChange?(textView)
        externalDelegate?.textViewDidChange?(textView)
    }
    
    func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        
        var internalAllowed = true
        var externalAllowed = true
        
        if let internalDel = internalDelegate, internalDel.responds(to: #selector(UITextViewDelegate.textView(_:shouldChangeTextIn:replacementText:))) {
            internalAllowed = internalDel.textView!(textView, shouldChangeTextIn: range, replacementText: text)
        }
        
        if let externalDel = externalDelegate, externalDel.responds(to: #selector(UITextViewDelegate.textView(_:shouldChangeTextIn:replacementText:))) {
            externalAllowed = externalDel.textView!(textView, shouldChangeTextIn: range, replacementText: text)
        }
    ...
    func textViewDidBeginEditing(_ textView: UITextView) {
        internalDelegate?.textViewDidBeginEditing?(textView)
        externalDelegate?.textViewDidBeginEditing?(textView)
    }
    
    func textViewDidEndEditing(_ textView: UITextView) {
        internalDelegate?.textViewDidEndEditing?(textView)
    ...
    func textViewShouldBeginEditing(_ textView: UITextView) -> Bool {
        ...
        if let internalDel = internalDelegate, internalDel.responds(to: #selector(UITextViewDelegate.textViewShouldBeginEditing(_:))) {
            internalResult = internalDel.textViewShouldBeginEditing!(textView)
        }

        var externalResult = true
        if let externalDel = externalDelegate, externalDel.responds(to: #selector(UITextViewDelegate.textViewShouldBeginEditing(_:))) {
            externalResult = externalDel.textViewShouldBeginEditing!(textView)
        }
    ...
    func textViewShouldEndEditing(_ textView: UITextView) -> Bool {
        var internalResult = true
        if let internalDel = internalDelegate, internalDel.responds(to: #selector(UITextViewDelegate.textViewShouldEndEditing(_:))) {
            internalResult = internalDel.textViewShouldEndEditing!(textView)
        }

        var externalResult = true
        if let externalDel = externalDelegate, externalDel.responds(to: #selector(UITextViewDelegate.textViewShouldEndEditing(_:))) {
            externalResult = externalDel.textViewShouldEndEditing!(textView)
        }
        return internalResult && externalResult
    }
}
